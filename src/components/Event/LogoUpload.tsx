...
const LogoUpload = () => {
  ...
  const [previewUrl, setPreviewUrl] = useState('https://images.unsplash.com/photo-1518770660439-4636190af475?w=200&h=200&fit=crop&crop=center');
    ...
    <>
      <Card className="relative group max-w-sm mx-auto overflow-hidden bg-gradient-to-br from-background to-muted/20 border border-border/50 hover:border-primary/30 transition-all duration-300 hover:shadow-lg hover:shadow-primary/10">
        <div className="p-6">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground">Click edit to change</p>
          </div>
        </div>
      </Card>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-xl font-semibold">Update Logo</DialogTitle>
            <DialogDescription>
              Choose a new logo for your company. Supported formats: JPG, PNG, SVG (max 5MB)
            </DialogDescription>
          </DialogHeader>
